...
import { Favorites, FavoritesEntity } from './favorites.interface';
import { Album } from '../albums/albums.interface';
import { ArtistEntity, Artists } from '../artists/artists.interface';

type Favs = {
  album: string[]; // favorite albums ids
  track: string[]; // favorite tracks ids
  artist: string[]; // favorite tracks ids
}
...
export class FavoritesService {
  ...
  add ( arg, id ) {
    ...
    if (arg === 'tracks') {
      try {
        const item = this.tracksService.getById( id )
      } catch {
      ...
      try {
        const item = this.albumsService.getById( id )
      } catch {
        throw new UnprocessableEntityException(HttpStatus.UNPROCESSABLE_ENTITY)
      }
      ...
      try {
        const item = this.artistsService.getById( id )
      } catch {
        throw new UnprocessableEntityException(HttpStatus.UNPROCESSABLE_ENTITY)
      }
    }
  ...
  remove( arg, id ) {
    ...
    removeFrom( arg, id ) {
      ...
      if ( this.inMemoryStore[arg].includes( id ) ) {
        this.inMemoryStore[arg] =
          this.inMemoryStore[arg].filter( value => value !== id )
      }
    }
